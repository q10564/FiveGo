...
void HttpListener::listen()
{
    ...
    if (!isListening())
    {
        qCritical("HttpListener: Cannot bind ip %s on port %i: %s", qPrintable(host), port, qPrintable(errorString()));
    }
    else {
        qDebug("HttpListener: %s Listening on port %i", qPrintable(host), port);
    }
}
